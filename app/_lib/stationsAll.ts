...
import { buildNoaaUrl, fetchNoaaUrl } from "@/app/_lib/noaa"
import { makeStationsError } from "@/app/_lib/stationsUtils"
import { check, initStorage, read, write } from "@/app/_lib/storage"
import type {
  NoaaTidePredStation,
  Station,
  StationsResponse,
...
export async function stationsAll(
  ...
  if (forceFetch) {
    ...
    console.log(
      `${__filename}: Forcing refresh of stations.json because no cache found`,
    )
  }

  const path = "/mdapi/prod/webapi/tidepredstations.json"
  console.log(
    `${__filename}: Fetching new stations.json from ${buildNoaaUrl(path)}`,
  )
  ...
  const stations: Station[] = stationList.map(
